...
public class Layer {
    ...
    public static interface LayerTileIterator {
        public void onTile(MapTile tile, int x, int y);
    }
    
	/**
	 * Underlying layer
	 */
	private MapTile[][] rows;
	...
	public Layer(String name,
		...
		this.name = name;
		this.rows = new MapTile[numberOfRows][];		
	...
	public void applyHeightMask() {
		for(int rowIndex = 0; rowIndex < this.rows.length; rowIndex++) {
			MapTile[] row = this.rows[rowIndex];
	 ...
	 * @param row
	 */
	public void addRow(int index, MapTile[] row) {
		this.rows[index] = row;		
		for(int i = 0; i < row.length;i++) {
			MapTile t = row[i];
			if(t!=null) {
	...
	/**
	 * Iterate over each {@link MapTile} in this {@link Layer}
	 * @param it
	 */
